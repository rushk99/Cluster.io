...
file_path = "../data/MTS_example1_25x25_flower_edited.xlsx"
...
# The hardness values
hardness_column = test1_sheet[hard_col_name][1:]
# The x values
x_column = test1_sheet[x_col_name][1:]
# The y values
y_column = test1_sheet[y_col_name][1:]
...
print(y_df)
print("\nBelow are the labeled data values... ")
...
num_clusters = len(np.unique(labeled_data_df))
if num_clusters > 20:
    num_clusters = 20
x_values = np.unique(x_df["Data"].values)
y_values = np.unique(y_df["Data"].values)
grid_size_x = int(x_values.size)
grid_size_y = int(y_values.size)
z_values = np.rot90(labeled_data_df["Data"].values.reshape(grid_size_x, grid_size_y))
...
cb = fig.colorbar(cf, ax=ax)
# Labels being set
cb.set_label("Labeled Values")
plt.title("Labeled Data VS Coordinate")
plt.xlabel("X Values (um)")
plt.ylabel("Y Values (um)")

plt.show()

plt.imshow(labeled_data_df["Data"].values.reshape(25, 25), origin='lower')
plt.show()
