...
class ExcelFileReaderHelper:
    ...
    def get_sheet(self, sheet_name, skip_first_row=True, use_cols="D:T", hard_col_name="HARDNESS",
        ...
        if nulls:
            test1_sheet = pd.read_excel(self.xls, sheet_name, usecols=use_cols)
        else:
            test1_sheet = pd.read_excel(self.xls, sheet_name, usecols=use_cols).dropna()
        ...
        return hard_df, modu_df, x_df, y_df
